...
- **ENGINE — current holdings in the context (look-ahead-free, backward-compatible).** `RebalanceContext` gained `holdings: dict[str,float]` (current WEIGHT per held symbol at decision time) + `held(sym)->float` and `held_symbols()->list[str]`; default `{}` so every existing stateless strategy is BYTE-IDENTICAL (a strategy that ignores `holdings` is unaffected — proven by golden test). The ENGINE drives holdings; the strategy only READS them (determinism preserved). `backtest._build_context` gained `holdings_shares`/`cash` kwargs: weight[s] = shares·`assets[s].price`/equity, equity = cash + Σ shares·`assets[s].price`, using the SAME decision-date close the strategy sees -> self-consistent + look-ahead-free (NEVER today's fill). The rebalance call site passes the loop's pre-fill book + cash. `live._run_one` threads the pre-rebalance `current` shares + `cash` into its `_build_context` the same way, so a promoted stateful strategy holds positions live identically. NO change to walkforward.py (each fold starts flat through run_backtest). The 12 monthly finalists + all prior hunts are untouched.

- **ROSTER — `edgefinder/engine/hunt_daily_r2.py` → `HUNT_DAILY_R2_SPECS` (stateful hysteresis).** All LONG-ONLY, FIXED params, daily schedule, look-ahead-honest. Helper `_hysteresis(ctx, ranked, cap, enter_rank, exit_rank)`: KEEP held names still inside the WIDE exit band, ADD non-held names in the NARROW entry band up to cap, equal-weight — a buffer-zone held name (enter_rank ≤ rank < exit_rank) is RETAINED (the no-churn property). Specs: `sh_mom_20` (K=10, enter top-10 by ret(20), hold until rank≥30); `sh_trend_hold` (≤20, enter price>200-EMA ∩ ret(126)>0, CLEAN STATE EXIT only when price<200-EMA — no reselection churn); `sh_high_break` (K=15, enter `_high_ratio(a,252)`≥0.99, exit when price<50-EMA); `sh_reversal_5` (K=10, enter most-negative ret(5), exit when its ret(5) rank recovers above 30); controls `sh_random_71`/`sh_random_73` (seeded hash order through the SAME hysteresis machinery -> turnover-MATCHED yardstick; buy_and_hold:SPY null also in-batch). Wired into `make_strategy_factory`. LANE (orchestrator fires; queue stays IDLE): `--universe top:500 --start 2021-06-01 --schedule daily --rebalance-band 0.01 --costed --div-adjust --bars-from r2 --holdout-start 2026-04-01 --record --total-return --warmup-days 290`, labels daily-r2:*. FIRST confirm turnover COLLAPSES vs daily-r1 (the hysteresis working) controls-first; then any SPY-beating fold-majority earns the three adversarial re-checks; write `reviews/DAILY-ROUND-2.md`.

- Tests: `tests/test_engine.py` (+backward-compat golden: stateless run byte-identical with holdings threaded; +holdings-correctness/look-ahead: weights = pre-fill shares·decision-close/equity, never today's fill) + `tests/test_hunt_daily_r2.py` (shape/honesty for every spec incl. held-book path; TURNOVER: sh_mom_20 vs stateless top-10 twin on identical seeded bars = 235→57 trades, −75.7%, and a buffer-zone held name is retained) + `tests/test_engine_live.py::TestStatefulHoldings` (live parity: the runner threads the book; a hold-decision produces NO churn). Full suite 727 green. `hunt/queue.json` stays IDLE.

**DAILY-DECISION ROUND 1 COMPLETE (2026-06-14) — 0 of 9, structural finding.** reviews/DAILY-ROUND-1.md. The no-trade band works (SPY null = 6 trades, -0.06pp), but every candidate is a stateless cross-sectional daily-reselected top-K basket on 500 names -> MEMBERSHIP rotates daily by construction -> the band (which only spares HELD names) can't help -> toll death, monotonic in turnover (reversal_1 -74pp/25k trades; trend_hold least-bad -4.56/12.7k; longer lookback=stickier=less loss). LESSON: 'analyze daily, trade when warranted' needs FIXED/SMALL universes OR a STATEFUL hold/exit interface (strategy sees its holdings), NOT stateless cross-sectional reselection. Through-line across intraday-r1/r2 + daily-r1: higher decision frequency on ROTATING baskets is dominated by turnover tolls; the validated edge lives at MONTHLY cadence (the 12 finalists). Also reshapes the blind agent: feed it current holdings so it can HOLD. Holdout 2026-04-01 sealed; no re-checks. NEXT (owner call): (a) small/fixed-universe daily lane, (b) stateful hold/exit model, or (c) the blind agent live-experimental with holdings. queue idle.

**DAILY-DECISION LANE + BLIND LLM AGENT (2026-06-14, v5.56.0) — built, PRE-REGISTERED, NOT YET RUN. INTRADAY PARKED.** Framing correction: the owner wants the system to analyze EVERY day whether to buy/sell/hold while letting each strategy hold for whatever its signal dictates (flips → swings → long-term). No new engine needed — `--schedule daily` already asks for target weights daily, and `--rebalance-band` makes "hold" FREE (a months-long position is re-evaluated daily and simply kept, trading zero times until the signal flips). So holding horizon is a per-strategy property, not the schedule. The two intraday rounds returned 0 finalists; that initiative is now PARKED (engine + minute asset retained, queue idle).

- **PART A — daily-decision mechanical roster (`edgefinder/engine/hunt_daily_r1.py` → `HUNT_DAILY_R1_SPECS`).** All LONG-ONLY cross-sectional equal-weight K=20, FIXED params, look-ahead-honest (decide on data ≤ decision-date close; engine fills next open, so any "gap" is the LAGGED gap = `open[-1]/close[-2]-1`, known at decision time — never today's open). Reuses hunt_r1 helpers (`_topk_ew`,`_vol`,`_dollar_vol`,`_high_ratio`) + `AssetView.ret(n)`. SPANS horizons: quick-flip `dr_reversal_1`/`dr_reversal_5`/`dr_gap_fade`/`dr_gap_cont`; swing `dr_mom_20`/`dr_high_break`/`dr_vol_breakout`; position `dr_trend_hold` (>200-EMA ∩ ret(126)>0 equal-weight — a slow basket the band holds for months, proving the loop covers long-term too); controls `dr_random_61`/`dr_random_67` (fresh-seed RandomK; the null buy_and_hold:SPY already exists, queue includes it). Wired into `make_strategy_factory`. LANE: `--universe top:500 --start 2021-06-01 --schedule daily --rebalance-band 0.01 --costed --div-adjust --bars-from r2 --holdout-start 2026-04-01 --record --total-return`, labels daily-r1:*. Honesty rig identical to the monthly hunt (null + randoms in-batch, all-three adversarial re-checks before any finalist claim). `hunt/queue.json` stays IDLE — the orchestrator fires the wave.

- **PART B — blind LLM judgment agent (`edgefinder/engine/llm_strategy.py`, `LLMStrategy`).** A new strategy TYPE whose signal is an LLM's buy/sell/hold judgment over the point-in-time context. ONE prompt per rebalance (whole universe → modest cost), back comes `{label: weight}`, normalized long-only (negatives dropped, sum>1 scaled to 1.0). **MEMORIZATION CAVEAT (the heart of it):** an LLM has memorized real market history through its training cutoff, so naming "AAPL"/"2026-04-15" would be look-ahead via training data. The BACKTEST payload is therefore ANONYMIZED: NO date (ordinal `day_index` only), tickers RELABELED A/B/C… (private decision-local map; real symbol never sent), prices normalized (last-20 closes → 100.0) + indicator values + ret(5/20/60). This REDUCES but cannot ELIMINATE memorization → the anonymized backtest is explicitly NON-AUTHORITATIVE. Decisions cached by `sha256(canonical anonymized payload)` (`llm_decision_cache`) so a re-run reproduces identical trades; every prompt+response logged to `llm_decision_log`. Reuses `agents/reasoning.py:call_claude_json` (factored out of `_call_claude_cli` — same `claude -p --output-format json`, CLAUDE_CODE_OAUTH_TOKEN, 180s, envelope+prose-strip parse). Default model = CHEAP `claude-haiku-4-5-20251001`. DEGRADES SAFELY: missing token / call / parse failure → {} all-cash, never crashes. **EXPERIMENTAL-LIVE IS THE REAL TEST:** in live paper trading the future genuinely does not exist, so the agent is blind by construction (no anonymization needed) — promote via `engine/promote.py --tier experimental` and judge it on the live-vs-lab scorecard over coming weeks, the honest home for a strategy whose backtest can't be fully trusted. Spec `llm_blind` / `llm_blind:<model>` wired into `make_strategy_factory`. DB: `llm_decision_cache`/`llm_decision_log` added to db/models.py + alembic `d4f8a1c93b27` (table-exists-guarded; orchestrator applies prod DDL separately).

- Tests: `tests/test_hunt_daily_r1.py` (shape/honesty, mirrors test_hunt_r1) + `tests/test_llm_strategy.py` (stubbed runner, NO real claude -p: anonymization hides ticker+date, labels map back, clamp+scale, cache-hit skips the 2nd call, missing-token/malformed → {}). Full suite 714 green. The 12 monthly finalists are untouched. Both `hunt/queue.json` and `intraday/queue.json` stay IDLE.

**INTRADAY ROUND 2 COMPLETE (2026-06-14) — 0 of 8 finalists, but a rich result.** reviews/INTRADAY-ROUND-2.md. Low-turnover pivot (enter-once hold, rebalance-band 0.01) worked: gap_go went from round-1 annihilation to -0.70pp. Both lanes: overnight reclaims ~+2.5-3pp of drift vs flat (anchor -2.45->-0.23; randoms ~-6->~-3 — measured, not alpha). gap_go-on is the one signal beating RANDOM (-0.70 vs ~-3.2 random floor; 10/23 folds) but STILL below the passive overnight SPY anchor (-0.23) -> beats random, not SPY. No candidate beats SPY; no re-checks; holdout sealed/unburned. TWO-ROUND CONCLUSION: no intraday strategy beats SPY net of tolls on liquid mega-caps; the only live signal (gap-up continuation) earns over random only by holding OVERNIGHT = a swing effect, not intraday alpha. ROUND-3 FORK (owner call): (1) accept null & park intraday; (2) different/less-efficient universe (higher tolls); (3) reframe gap-continuation as an overnight SWING strat on the DAILY fleet. Awaiting owner direction.
...
**INTRADAY ROUND 2 PRE-REGISTERED (2026-06-14, v5.55.0) — the LOW-TURNOVER pivot. NOT YET RUN (queue IDLE; build complete, wave fires separately).** Round 2 tests the turnover hypothesis directly with ENTER-ONCE, HOLD-TO-CLOSE families: each returns the SAME stable basket on every decision bar from its entry trigger onward, so it pays ~2 tolls/day (entry + MOC flatten) instead of Round-1's hundreds. Pre-registration: `edgefinder/engine/intraday_roster_r2.py` (module docstring = the statement; FIXED params; the Round-1 lesson; the two lanes). Specs in `INTRADAY_R2_SPECS` (all LONG-ONLY, equal-weight K=5): GAP `iro_gap_fade`/`iro_gap_go` (thresh=0.005; gap measured off the FIXED session_open vs prev_close, so the basket is constant all session — gap_go DROPS R1's non-stable price>open gate); BREAKOUT `iro_orb` (m=30; uses session_high>OR_high which is MONOTONE — once broken, stays in — so membership is stable, not the churny price>OR_high); MORNING-TREND `iro_morning_trend` (m=30; ranks by the FIXED m-th-bar close vs session open, `closes(bars_since_open)[m-1]/session_open-1`, frozen once bar m exists); CONTROLS `iro_random_201`/`iro_random_203` (hash(seed,date,symbol), fixed per session — the low-turnover floor recalibrated for this cadence). ENGINE CHANGE (v5.55.0): `run_intraday_backtest` (+walkforward, + CLI `--rebalance-band`, default 0.0) gained a no-trade rebalance band mirroring the DAILY engine's `_execute_to_target` EXACTLY (skip a name's re-true when `abs(delta)*open_px < band*equity` UNLESS it opens/fully-closes the position). Default 0.0 = exact re-true (preserves every Round-1 result bit-for-bit). Round 2 runs at `--rebalance-band 0.01` — without it, an enter-once strategy's identical target still integer-share-churns as the equity mark drifts. THE TWO LANES are a RUN flag (the queue passes `--flatten-at-close` OR `--hold-overnight`), NOT a strategy difference — each spec is registered ONCE and run twice. Sealed holdout 2026-04-01 (unchanged). Tests: `tests/test_intraday_roster_r2.py` (24): every spec builds/legal-weights/{}-at-close, gap picks gap-downs/ups only and the basket is STABLE across consecutive bars, ORB membership is monotone (stays in after a pullback), morning-trend ranking is fixed after bar m, random is deterministic per (seed,date), + engine band tests (band cuts churn >2x, band never blocks the open buy or MOC flatten, band=0.0 reproduces the no-band result bit-for-bit, band threads through walkforward). To RUN: edit `intraday/queue.json` (currently `{"wave":"idle","runs":[]}`) with the R2 specs × both lanes at `--rebalance-band 0.01 --holdout-start 2026-04-01` and commit — the orchestrator fires the wave.
