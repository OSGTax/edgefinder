...
Technical entry: Volume spike (2x+ = squeeze trigger), RSI crossing above 40,
EMA crossover (day).
...
class PapaStrategy(BaseStrategy):
    ...
    def preferred_signals(self) -> set[str]:
        return {"volume_spike", "rsi_oversold", "ema_crossover_day"}
    ...
    def generate_signals(self, bars: dict[str, pd.DataFrame]) -> list[Signal]:
        ...
        for ticker, df in bars.items():
            ...
            for ts in trade_signals:
                ...
                if price <= 0:
                    continue
                # Volume spike is extra important for squeeze plays
                has_volume_spike = any(
                    (ind.get("name") if isinstance(ind, dict) else ind) == "volume_spike"
                    for ind in (ts.indicators if isinstance(ts.indicators, list)
                               else [ts.indicators] if ts.indicators else [])
                )
                if has_volume_spike:
                    confidence = min(100.0, confidence + 5.0)  # Slight boost for volume confirmation
                ...
                if confidence < settings.SIGNAL_MIN_CONFIDENCE_TO_TRADE:
                    continue
                risk_pct = 0.08  # Widest stop — squeeze plays are volatile
                ...
                meta = {
                    ...
                    "volume_spike_detected": has_volume_spike,
                }
