"""
EdgeFinder Strategy: ALPHA — Lynch Fast Grower
================================================
Small/mid-cap companies growing earnings 20-50%/yr with low PEG ratios.
Peter Lynch's bread and butter — catches multi-bagger profiles early.

Fundamental screening: lynch_category == fast_grower, PEG < 1.2,
earnings growth >= 20%, low debt.

Technical entry: EMA crossovers (day), MACD crossover, volume spike.
Sentiment gate: blocks trades on strongly negative news.
"""

import logging

import pandas as pd

from config import settings
from modules.strategies.base import (
    BaseStrategy,
    StrategyRegistry,
    Signal,
    TradeNotification,
    MarketRegime,
)
from modules.signals import compute_indicators, generate_signals as detect_signals

logger = logging.getLogger(__name__)


def _has_indicator(ts_indicators, name: str) -> bool:
    """Check if a named indicator fired in a signal's indicator data."""
    if isinstance(ts_indicators, dict):
        return name in ts_indicators
    elif isinstance(ts_indicators, list):
        return any(ind.get("name") == name for ind in ts_indicators if isinstance(ind, dict))
    return False


@StrategyRegistry.register("alpha")
class AlphaStrategy(BaseStrategy):
    """Lynch Fast Grower — small/mid-cap high-growth, low PEG."""

    @property
    def name(self) -> str:
        return "alpha"

    @property
    def version(self) -> str:
        return "1.0.0"

    @property
    def preferred_signals(self) -> set[str]:
        return {"ema_crossover_day", "macd_crossover", "bollinger_breakout", "near_52w_high"}

    def init(self) -> None:
        self._watchlist: list[str] = []
        self._scores: dict[str, dict] = {}
        self._trades_log: list[TradeNotification] = []
        self._use_sentiment: bool = True
        self._atr_multiplier: float = 2.0
        self._fallback_risk_pct: float = 0.05
        self._52w_high_confidence_boost: float = 5.0
        logger.info("Alpha (Lynch Fast Grower) strategy initialized")

    def qualifies_stock(self, stock_data: dict) -> bool:
        """Fast grower with low PEG, strong earnings, manageable debt."""
        return (
            stock_data.get("lynch_category") == "fast_grower"
            and (stock_data.get("peg_ratio") or 999) < 1.2
            and (stock_data.get("earnings_growth") or 0) >= 0.20
            and (stock_data.get("debt_to_equity") or 999) < 0.8
        )

    def set_watchlist(self, scored_stocks: list[dict]) -> None:
        self._watchlist = []
        self._scores = {}
        for stock in scored_stocks:
            if self.qualifies_stock(stock):
                ticker = stock["ticker"]
                self._watchlist.append(ticker)
                self._scores[ticker] = stock
        logger.info(f"Alpha watchlist: {len(self._watchlist)} stocks")

    def get_watchlist(self) -> list[str]:
        return list(self._watchlist)

    def generate_signals(self, bars: dict[str, pd.DataFrame]) -> list[Signal]:
        signals = []
        for ticker, df in bars.items():
            if df is None or df.empty:
                continue
            snapshot = compute_indicators(df, ticker=ticker)
            if snapshot is None:
                continue
            trade_signals = detect_signals(snapshot)
            if not trade_signals:
                continue
            for ts in trade_signals:
                if ts.signal_type != "BUY":
                    continue
                signal_names = (
                    set(ts.indicators.keys())
                    if isinstance(ts.indicators, dict)
                    else {ind.get("name") for ind in ts.indicators if isinstance(ind, dict)}
                )
                if not signal_names & self.preferred_signals:
                    continue
                confidence = ts.confidence
                price = ts.price or (
                    float(df.iloc[-1]["Close"])
                    if "Close" in df.columns
                    else float(df.iloc[-1].get("close", 0))
                )
                if price <= 0:
                    continue
                if self._use_sentiment:
                    try:
                        from modules.sentiment import gate_trade
                        action, adjusted_confidence, _ = gate_trade(ticker, confidence)
                        if action == "BLOCK":
                            logger.info(f"[alpha] {ticker}: blocked by sentiment")
                            continue
                        confidence = adjusted_confidence
                    except Exception as e:
                        logger.debug(f"[alpha] Sentiment gate error: {e}")

                # Confidence boost: near 52-week high = momentum continuation
                has_52w_high = _has_indicator(ts.indicators, "near_52w_high")
                if has_52w_high:
                    confidence = min(100.0, confidence + self._52w_high_confidence_boost)

                if confidence < settings.SIGNAL_MIN_CONFIDENCE_TO_TRADE:
                    continue

                # ATR-based dynamic stop-loss
                if snapshot.atr and snapshot.atr > 0:
                    stop_loss = round(price - (snapshot.atr * self._atr_multiplier), 2)
                else:
                    stop_loss = round(price * (1 - self._fallback_risk_pct), 2)
                target = round(price + (price - stop_loss) * 2.0, 2)
                meta = {
                    "strategy": "alpha",
                    "indicators": ts.indicators,
                    "trade_reason": ts.reason,
                    "52w_high_boost_applied": has_52w_high,
                }
                score_info = self._scores.get(ticker, {})
                if score_info:
                    meta["lynch_score"] = score_info.get("lynch_score")
                    meta["lynch_category"] = score_info.get("lynch_category")
                    meta["peg_ratio"] = score_info.get("peg_ratio")
                signals.append(Signal(
                    ticker=ticker,
                    action="BUY",
                    entry_price=price,
                    stop_loss=stop_loss,
                    target=target,
                    confidence=confidence,
                    trade_type=ts.trade_type,
                    metadata=meta,
                ))
        return signals

    def on_trade_executed(self, notification: TradeNotification) -> None:
        self._trades_log.append(notification)
        logger.info(f"[alpha] Trade: {notification.action} {notification.ticker} @ ${notification.entry_price:.2f}")

    def on_market_regime_change(self, regime: MarketRegime) -> None:
        if regime.trend == "bull":
            logger.info("[alpha] Bull market — favorable for fast growers")
        elif regime.trend == "bear":
            logger.info("[alpha] Bear market — caution, fast growers hit hardest")
        else:
            logger.info(f"[alpha] Market regime: {regime.trend}/{regime.volatility}")

    def on_strategy_pause(self, reason: str) -> None:
        logger.warning(f"[alpha] Strategy paused: {reason}")
