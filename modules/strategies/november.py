"""
EdgeFinder Strategy: NOVEMBER — Turnaround Play
=================================================
Lynch turnaround-classified stocks showing early recovery signs.
Positive FCF + liquidity = survival confirmed.
...
@StrategyRegistry.register("november")
class NovemberStrategy(BaseStrategy):
    """Turnaround Play — recovery + survival proof."""
    ...
    def qualifies_stock(self, stock_data: dict) -> bool:
        """Turnaround with survival evidence: positive FCF, liquid."""
        return (
            stock_data.get("lynch_category") == "turnaround"
            and (stock_data.get("current_ratio") or 0) >= 1.5
            and (stock_data.get("fcf_yield") or 0) >= 0.03
            and (stock_data.get("price_to_tangible_book") or 999) < 2.5
        )
    ...
    def set_watchlist(self, scored_stocks: list[dict]) -> None:
        ...
        logger.info(f"November watchlist: {len(self._watchlist)} stocks")
            ...
            for ts in trade_signals:
                ...
                if self._use_sentiment:
                    try:
                        ...
                        if action == "BLOCK":
                            logger.info(f"[november] {ticker}: blocked by sentiment")
                            continue
                ...
                if score_info:
                    ...
                    meta["current_ratio"] = score_info.get("current_ratio")
    ...
    def on_trade_executed(self, notification: TradeNotification) -> None:
        ...
        logger.info(f"[november] Trade: {notification.action} {notification.ticker} @ ${notification.entry_price:.2f}")
    ...
    def on_strategy_pause(self, reason: str) -> None:
        logger.warning(f"[november] Strategy paused: {reason}")
